...
module.exports = function(cb) {
  ...
  self.describe(function afterDescribe(err, attrs) {
    ...
    self.query.find(queryCriteria, function(err, existingData) {
      ...
      //
      backupData = _.cloneDeepWith(existingData, function dealWithBuffers(val) {
      ...
      var attrsAsArray = _.reduce(_.cloneDeepWith(attrs), function(memo, attrDef, attrName) {
        attrDef.name = attrName;
        memo.push(attrDef);
        return memo;
      }, []);
      var uniqueAttrs = _.filter(attrsAsArray, {unique: true});
      async.each(uniqueAttrs, function(uniqueAttr, each_cb) {
        var uniqueData = _.uniq(_.map(existingData, uniqueAttr.name));
        ...
        if (cleansedUniqueData.length < cleansedExistingData.length) {
          ...
          prompt.get(['y/n'], function(err, results) {
            ...
            if (wasConfirmedByUser) {
              ...
              var destroyCriteria = {};
              destroyCriteria[pk] = _.map(diff, pk);
              // console.log(diff, '\n', destroyCriteria);
